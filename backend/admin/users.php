...
if (isset($_GET['delete'])) {
    ...
    // Security Patch: Verify board ownership for students
    $verify = $pdo->prepare("
        SELECT u.user_id FROM users u
        LEFT JOIN classes c ON u.class_id = c.class_id
        WHERE u.user_id = ? AND (c.board_type = ? OR u.user_type = 'teacher' OR u.board = ? OR u.board_type = ?)
    ");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO users (name, email, mobile, phone, password, user_type, class_id, subscription_status, board_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, NOW())");
        $stmt->execute([$name, $email, $mobile, $mobile, $password, $type, $class_id, $selected_board]);
...
// Apply Board Filter
$query_str .= " AND (c.board_type = ? OR u.user_type = 'teacher' OR u.board_type = ?)";
...
<head>
    ...
    <title>Manage Users - Veeru Admin</title>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <form method="POST">
                <div class="form-grid">
                    ...
                    <input type="tel" name="mobile" placeholder="Mobile Number" required pattern="[0-9]{10}" title="Ten digit mobile number">
        ...
        <div class="card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h2><i class="fa-solid fa-users"></i> All Users (<?php echo $board_name; ?>)</h2>
                <form method="GET" style="display: flex; gap: 10px; align-items: center;">
                    <input type="text" name="search" placeholder="Search users..." value="<?php echo htmlspecialchars($search_query); ?>" style="width: 250px; margin-bottom: 0;">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if(!empty($user['school_name'])): ?>
                                <br><small style="color:#666"><i class="fa-solid fa-school"></i> <?php echo htmlspecialchars($user['school_name']); ?></small>
                            <?php endif; ?>
                        </td>
                        <td><?php echo htmlspecialchars($user['email']); ?></td>
                        <td><?php echo htmlspecialchars($user['mobile'] ?: ($user['phone'] ?: '-')); ?></td>
                        <td>
                            <span class="badge" style="background: <?php echo $user['user_type'] == 'teacher' ? '#6366f1' : '#10b981'; ?>; color: white; padding: 4px 8px; border-radius: 4px; font-size: 11px;">
                        ...
                        <td>
                            <a href="?delete=<?php echo $user['user_id']; ?>" class="btn-delete" onclick="return confirm('Are you sure?')" style="padding: 6px 10px; font-size: 12px;"><i class="fa-solid fa-trash"></i> Delete</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
